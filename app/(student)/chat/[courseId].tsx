...
import {
  ...
  useUploadChatAttachment, useChatChannels, mergeChatMessage, dropChatMessage,
} from '@/hooks/useChat';
...
/**
 * One course room, as a STUDENT sees it (Course Group Chat spec).
 *
 * It is dressed as the messenger they already use — papered ground, tinted bubbles with
 * tails, day chips, a pill composer whose mic turns into a send arrow — because this is
 * meant to REPLACE the class WhatsApp group, and a room that feels foreign gets abandoned
 * for the one that doesn't.
 *
 * What the screen never decides: who is a member, whether this student may post, what a
...
export default function StudentChatRoom() {
  ...
  const courseId = parseInt(String(params.courseId ?? '0'), 10) || 0;
  ...
  // Long-press menu. Mine → delete. Someone else's → report, or hide them from my view.
  const onMessageMenu = (m: ChatMessage) => {
    ...
    const buttons: Array<{ text: string; style?: 'cancel' | 'destructive'; onPress?: () => void }> = [
      { text: t('chat.report'), onPress: () => { setReporting(m); setReportReason(null); setReportNote(''); } },
    ];
  ...
  if (room.isLoading && !room.data) {
    return (
      <View style={{ flex: 1, backgroundColor: chat.wallpaper, justifyContent: 'center', alignItems: 'center' }}>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: chat.wallpaper }}>
      {/* Header — avatar, name, members, and whether the room is live right now. */}
      <View
        style={{
          flexDirection: 'row', alignItems: 'center', gap: spacing.sm,
          paddingTop: insets.top + spacing.sm, paddingBottom: spacing.sm, paddingHorizontal: spacing.sm,
      ...
      >
        ...
        <TouchableOpacity onPress={toggleNotify} disabled={setNotify.isPending} style={{ width: 44, height: 44, justifyContent: 'center', alignItems: 'center' }} accessibilityRole="button" accessibilityLabel={notify ? t('chat.notify_on') : t('chat.notify_off')}>
          <Icon name={notify ? 'bell' : 'bellOff'} size={22} color={notify ? '#fff' : 'rgba(255,255,255,0.6)'} outline />
        </TouchableOpacity>
      </View>

      {(data.locked || data.announcements_only) ? (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, backgroundColor: '#FDF3D8', paddingHorizontal: spacing.lg, paddingVertical: spacing.sm }}>
          <Icon name={data.locked ? 'lock' : 'info'} size={15} color="#8A6109" outline />
          <Text style={{ flex: 1, fontFamily: fonts.medium, fontSize: 12.5, color: '#6B4A05' }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          >
            {messages.length >= (data.page_size || 50) && !noMoreOlder ? (
              <TouchableOpacity onPress={loadOlder} disabled={loadingOlder} style={{ alignSelf: 'center', paddingVertical: 7, paddingHorizontal: spacing.lg, borderRadius: radius.full, backgroundColor: chat.chip, marginBottom: spacing.sm }}>
            ...
            {messages.length === 0 ? (
              <View style={{ alignItems: 'center', paddingVertical: spacing.xl4 }}>
                <View style={{ backgroundColor: chat.chip, borderRadius: radius.lg, paddingVertical: spacing.lg, paddingHorizontal: spacing.xl, maxWidth: 300 }}>
            ...
            {upload.isPending ? (
              <View style={{ alignSelf: 'flex-end', flexDirection: 'row', alignItems: 'center', gap: spacing.sm, backgroundColor: chat.bubbleMine, borderRadius: radius.lg, paddingVertical: 8, paddingHorizontal: spacing.md, marginTop: spacing.sm, opacity: 0.85 }}>
                <ActivityIndicator size="small" color={chat.metaOnMine} />
                <Text style={{ fontFamily: fonts.medium, fontSize: 13, color: chat.metaOnMine }}>{t('chat.uploading')}</Text>
              </View>
            ) : null}
          </ScrollView>
